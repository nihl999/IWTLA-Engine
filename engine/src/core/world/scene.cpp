  ...
  if (ImGui::TreeNode("Entity List: ")) {
    ...
    world.each([&](flecs::entity e, Transform &t, DirectionalLight &light) {
      ...
      if (ImGui::TreeNode(&dirlightes, "Dir Light %d", dirlightes)) {
        ImGui::SliderFloat3("Dir", &light.direction[0], -1.0f, 1.0f, "%.2f");
...
void Scene::RegisterMovementSystem() {
  flecs::system move_sys =
      world.system<Transform, Velocity>().each([](Transform &t, Velocity &v) {
...
void Scene::RegisterTransformMatrixCalcSystem() {
  ...
  flecs::system move_sys =
      world.system<Transform>().each([identity](Transform &t) {
        ...
        glm::mat4 rotationMatrix =
            glm::rotate(identity, glm::radians(xAngle), glm::vec3(1, 0, 0));
        rotationMatrix *=
            glm::rotate(identity, glm::radians(yAngle), glm::vec3(0, 1, 0));
        rotationMatrix *=
...
void Scene::RegisterRenderModels() {
  auto render_sys =
      world.system<Transform, ECSComponents::Model, Renderable>().each(
          [this](Transform &t, ECSComponents::Model &m, Renderable r) {
            ...
            for (Graphics::Mesh mesh : model->meshes) {
              ...
              renderer.UniformFMat4("view_projection_matrix",
                                    camera.getViewMatrix());
              {
                // NOTE: Directional Light
                renderer.UniformF1("dirLight.intensity", lightC->intensity);
                renderer.UniformFVec3("dirLight.direction", lightC->direction);
                renderer.UniformFVec3("dirLight.color", lightC->color);
              }
              {
                // NOTE: Point Lights
                // todo ongod fuck fuck -- even worse
                u32 index = 0;
                auto f = world.filter<Transform, PointLight>();
                f.each([&index, &renderer](Transform &t, PointLight &p) {
                  if (index < 10) {
                    renderer.UniformF1("pointLights[" + std::to_string(index) +
                                           "].intensity",
                                       p.intensity);
                    renderer.UniformFVec3("pointLights[" +
                                              std::to_string(index) + "].color",
                                          p.color);
                    renderer.UniformFVec3(
                        "pointLights[" + std::to_string(index) + "].position",
                        t.position);
                    renderer.UniformF1("pointLights[" + std::to_string(index) +
                                           "].linear",
                                       p.linear);
                ...
                u32 count = f.count();
                renderer.UniformU1("qtd_point_light", count);
              }
              renderer.UniformFVec3("cam_pos", camera.position);
              renderer.UniformFMat4("model_matrix", t.modelMatrix);
